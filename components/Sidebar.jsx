...
import { Home, Search, Library, Plus, ArrowRight, Heart, Sparkles } from 'lucide-react';
    ...
    <div className="flex flex-col h-full gap-2 relative z-10">
      ...
      <nav className="spotify-panel flex flex-col p-4 py-3 shrink-0 bg-[#121212]/70 backdrop-blur-xl border border-white/5">
        ...
        <NavLink
          to="/"
          className={({ isActive }) =>
            `flex items-center gap-4 px-3 py-3 rounded-md font-black transition-all duration-300 ${
              isActive ? 'text-white bg-white/5 shadow-inner' : 'text-subdued hover:text-white hover:bg-white/5'
            }`
          }
        >
          {({ isActive }) => (
        ...
        <form onSubmit={handleSearch} className="mt-4 flex items-center gap-2">
          <input
            ...
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full bg-black/40 text-white text-sm px-3 py-2 rounded-md border border-white/10 focus:outline-none focus:border-spotify-green"
          />
          <button
            type="submit"
            className="bg-spotify-green text-black px-3 py-2 rounded-md text-sm font-bold hover:scale-105 transition"
          >
            Go
          </button>
        </form>
      ...
      <div className="spotify-panel flex-1 flex flex-col min-h-0 bg-[#121212]/70 backdrop-blur-xl border border-white/5">
        <header className="p-4 py-4 flex items-center justify-between">
          <button className="flex items-center gap-3 text-subdued hover:text-white transition-all font-black">
            <Library size={24} />
            <span className="text-[16px] tracking-tight">Your Library</span>
          </button>
          <div className="flex items-center gap-2">
            <button className="text-subdued hover:text-white p-2 rounded-full">
              <Plus size={20} />
            </button>
            <button className="text-subdued hover:text-white p-2 rounded-full">
              <ArrowRight size={20} />
        ...
        <div className="px-4">
          ...
          >
            <div className="w-12 h-12 bg-gradient-to-br from-indigo-600 to-emerald-400 rounded-md flex items-center justify-center text-white">
              <Heart fill="white" size={20} />
            </div>
            <div className="flex flex-col truncate">
              <span className="font-black text-[14px] text-white truncate">
                Liked Songs
              </span>
              <span className="text-[12px] text-subdued font-bold truncate">
                Playlist
              </span>
            </div>
          </NavLink>
